# Author details
## Script name: Figure 2.R
## Purpose of the script: To visualize the correlation plot with regression line from the DNA methylation data mentioned in the Das et al (2021), Epigenetics
...
figure2 <- ggarrange(B1, B2, B3, B4, B5, B1_CD3, B2_CD3, B3_CD3, B4_CD3, B5_CD3, 
  ...
  annotate_figure(figure2,
                  left = text_grob("Bronchoalveolar Lavage (BAL)", 
                    color = "black", family = "Times New Roman", 
                      size = 16, 
                        rot = 90),
                  bottom = text_grob("Sputum Induction (SI)", color = "black", 
  ...
  figure2 <- ggarrange(B1_1, B2_1, B3_1, B4_1, 
                        B5_1, B1_CD3_1, B2_CD3_1, B3_CD3_1, 
                          B4_CD3_1, B5_CD3_1 + rremove("x.text") + rremove("y.text"), 
                            labels = c("B1", "B2", "B3", "B4", "B5", "B1", "B2", "B3", "B4", "B5"),
                              ncol = 5, nrow = 2)
  annotate_figure(figure2,
                left = text_grob("Bronchoalveolar Lavage (BAL)", 
                  color = "black", 
                    family = "Times New Roman", 
                      size = 14, 
                        rot = 90),
                bottom = text_grob("Sputum Induction (SI)", 
                  color = "black", 
                    family = "Times New Roman", 
                      size = 14))
...
## Alternative to regression plot using the caret package with linear regression model analysis
  ...
  p1 <- ggplot(data, aes(x = B1_SPU_HLDADR, y = B1_LAV_HLDADR)) +
        geom_point() +
        stat_smooth()
  p1 + theme(text = element_text(family = "Times New Roman", 
          face = "bold", 
            size = 14), 
...
# install.packages("ggpointdensity")
  suppressMessages(suppressWarnings(library(ggpointdensity)))
  ...
  p2 <- ggplot(data, aes(x = B5_SPU_HLDADR, y = B5_LAV_HLDADR)) + 
        ...
        scale_color_viridis_c() +
	      stat_smooth()
  p2 + theme(text = element_text(family = "Times New Roman", 
          face = "bold", 
            size = 14), 
        axis.title = element_text(size = 12),
        axis.text = element_text(size = 12), axis.line = element_line(size = 1),
...
suppressMessages(suppressWarnings(library("ggsubplot")))
...
ggplotRegression <- function (fit) {
  require(ggplot2)
  # require all correlation information
